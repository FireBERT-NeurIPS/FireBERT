...
USE_multiplier = random.randint(9, 15) # 3
stop_words = True
perturb_words = random.randint(10, 25) # 5
candidates_per_word = 10 #random.randint(3, 15) #10
total_alternatives = 4 #random.randint(3, 20) # 5
...
switch_model = LightningBertForSequenceClassification(load_from = 'resources/models/IMDB/pytorch_model.bin', 
                                                      processor = ImdbProcessor(), 
                                                      hparams = {'batch_size': 6 })
switch_model.cuda()

model = FireBERT_FCT(switch_model=switch_model, processor=ImdbProcessor(hparams=proc_hparams), hparams=hparams)

processor = model.get_processor()

train_dataset, train_examples = processor.load_and_cache_examples("data/IMDB", example_set='train')
...
test_dataset = load_examples('data/IMDB/generated/imdb_adversarial_samples_for_dev')
model.set_test_dataset(test_dataset)
trainer = pl.Trainer(gpus=num_gpus)
trainer.test(model)
...
fname = str("results/fct/imdb-hparams-results.txt")
f = open(fname, "a")
f.write(results)
f.write( "\n")
f.close()
